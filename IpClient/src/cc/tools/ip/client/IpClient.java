...
public final class IpClient {
	...
	public IpClient() {
		...
		try {
			...
			System.err.println("exception: getting RSA KeyFactory " + e.getClass().getName() + ": " + e.getMessage());
	...
	private boolean getPublicIpFromServer(StringBuilder ipOut) {
		...
		try {
			int code = doPut(IpConfig.getIpConfig().getGetUrl(), response, data);

			if (code != 200) {
				System.err.println("error: call to doPut failed - code " + Integer.toString(code));
				return false;
			}
	...
	private boolean setPublicIpOnFromServer(String ip) {
		...
		try {
			int code = doPut(IpConfig.getIpConfig().getSetUrl(), response, data);
	...
	private static int doGet(String url, StringBuilder response) throws Exception {
