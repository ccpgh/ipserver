...
public final class IpServer extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		if (_keyFactory == null) {
			Logger.getLogger(IpServer.class.getName()).log(Level.SEVERE, "key factory init failed");
			return;
		}
	...
	private String encryptBase64WithPublicKey(String plainText) {
		if (plainText == null) {
			Logger.getLogger(IpServer.class.getName()).log(Level.SEVERE, "null parameter");
			return null;
		}
	...
	boolean getAllowedClients(String clientIp, StringBuilder buffer) {
		...
		if (isAllowedLocalhost) {
			clientIps.add(IpServer.CONST_LOCALHOST_IP_STRING);
		} else if (isAllowedClientIp && !clientIps.contains(clientIp)) {
	...
	void addClientIp(String clientIp) {
    	...
    	synchronized(_clientIps) {
    		if (IpServer.CONST_LOCALHOST_IP_STRING.compareTo(clientIp) != 0) {
	...
	private void serviceSetEndpoint(HttpServletRequest request, HttpServletResponse response)
		...
		if (formattedData != null && !formattedData.isEmpty()) {
			...
			Logger.getLogger(IpServer.class.getName()).log(Level.INFO, "added " + clientIp);
			response.setStatus(HttpURLConnection.HTTP_OK);
	...
	Key getKeyFromURI(String uri, Class<?> keyClass, String pattern, String generateMethodName) {
		...
		if (!loadURI(uri, buffer)) {
			Logger.getLogger(IpServer.class.getName()).log(Level.SEVERE, "unable to loadFile " + uri);
			return null;
		}
	...
	private boolean loadURI(String uri, StringBuilder buffer) {
		...
		try {
			...
			if (configStream == null) {
				Logger.getLogger(IpServer.class.getName()).log(Level.SEVERE,
						"failed to get InputStream for '" + uri + "'");
				return false;
			}
